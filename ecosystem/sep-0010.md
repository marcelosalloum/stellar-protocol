...
`client_domain` | string | (optional) The home domain hosting the client's `stellar.toml`. Only used for [Verifying Client Application Identity](#verifying-client-application-identity). **Servers should simply ignore this parameter if client attribution is not supported.**
...
* `transaction`: an XDR-encoded Stellar transaction with the following:
  ...
  * operations:
    ...
    * subsequent operations, order unimportant:
      ...
      * (optional) `manage_data(source: client_organization_account, key: 'client_domain', value: client_domain)`
        ...
        * the `client_organization_account` is the `SIGNING_KEY` from the `client_domain`'s [SEP-1 stellar.toml](sep-0001.md)
...
1. The SEP-10 server generates the challenge transaction with an additional Manage Data operation containing the `client_domain` key and parameter value passed.
...
1. The SEP-10 server uses the `client_domain` parameter to verify that the client organization's `SIGNING_KEY` signed the challenge transaction

**Note:** SEP-10 servers may keep an internal mapping of `client_domain` --> `SIGNING_KEY` for each known client organization. This allows the SEP-10 server to only make requests when the client organizations signature doesn't match the `SIGNING_KEY` it maps to. In this case, the SEP-10 server would check to ensure they had the correct `SIGNING_KEY` value and reject the request if it does. If the `SIGNING_KEY` value has changed, the SEP-10 server would verify the client organization's signature was generated using the new `SIGNING_KEY`. **It is highly recommended to only make requests to `client_domain`s of known client organizations. Making requests to unknown domains may slow down or halt the SEP-10 server's ability to verify challenge transactions.**
