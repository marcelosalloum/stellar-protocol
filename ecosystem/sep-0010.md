...
`client_domain` | string | (optional) The home domain hosting the client's `stellar.toml`. Only used for [Verifying Client Application Identity](#verifying-client-application-identity). **Servers should simply ignore this parameter if client attribution is not supported, or not supported for the specific client domain passed.**
...
* `transaction`: an XDR-encoded Stellar transaction with the following:
  ...
  * operations:
    ...
    * subsequent operations, order unimportant:
      ...
      * (optional) `manage_data(source: client_signing_key, key: 'client_domain', value: client_domain)`
        * Only add this operation if the client passed a `client_domain` parameter in the request and the server supports [Verifying Client Application Identity](#verifying-client-application-identity)
        * The `client_signing_key` is the `SIGNING_KEY` from the `client_domain`'s [SEP-1 stellar.toml](sep-0001.md)
...
* verify that transaction operations:
  * are all [Manage Data](https://www.stellar.org/developers/guides/concepts/list-of-operations.html#manage-data) operations
  * have non-null source accounts
...
* if the transaction contains a Manage Data operation with the key `client_domain`:
  * verify that the client domain included as the value has a `stellar.toml` file with a `SIGNING_KEY` used to sign the challenge. An internal mapping from client domain to SIGNING_KEY may be used.
...
* `client_domain` - (optional) a nonstandard JWT claim containing the client organization's home domain, specified in a Manage Data operation if the `client_domain` `GET` parameter was used (see [Verifying Client Application Identity](#verifying-client-application-identity))
...
### Verifying Client Application Identity

A server may want to attribute each HTTP request to a specific client application. For example, a server and client may agree to change fees for the users of that client. SEP-10 offers an optional variation to the flow outlined in the [Abstract](#abstract) to enable this use case. 

In order to use this optional feature, the organization that owns the client application must host a [SEP-1 stellar.toml](sep-0001.md) file containing a `SIGNING_KEY` attribute. This allows the SEP-10 server to verify that the challenge returned by the client is signed with the `SIGNING_KEY`. This proves that the client is associated with the client domain. The SEP-10 server can now attribute requests made with the resulting JWT to the client domain that signed the challenge.

While SEP-10 servers may chose to verify all client applications who request attribution, it is highly recommended to only verify a specific set of client applications and ignore attribution requests from other clients. **Making requests to unknown domains may slow down or halt the SEP-10 server's ability to respond to other requests.**

Outlined below is the flow between client applications and SEP-10 servers when attribution is requested:

1. The client makes the initial `GET <WEB_AUTH_ENDPOINT>` request with an added `?client_domain=example.com` parameter and value
1. If the SEP-10 server supports attribution requests for the specified `client_domain`, it fetches the `SIGNING_KEY` from the `client_domain`'s [SEP-1 stellar.toml](sep-0001.md) and generates the challenge transaction with an additional Manage Data operation as described in the [Response](#response) section, referred to here as the `client_domain` Manage Data operation
1. The client checks if the SEP-10 server returned a challenge transaction with the `client_domain` Manage Data operation. If it did not, the client aborts the flow outlined here and continues the authentication process as if attribution was not requested
1. The client obtains a signature generated using its organization's `SIGNING_KEY` and adds it to the challenge transaction. How this happens is up to the client. However:
  1. It is highly recommended that the client-side application does not hold the secret key for the `SIGNING_KEY`
  1. Instead, an internal service should accept a request from the organization's client application, verify the challenge sent, and generate the signature if valid
1. The client signs the challenge as it normally would with the user's Stellar account signer secret key(s)
1. The client makes the `POST <WEB_AUTH_ENDPOINT>` request with the signed transaction
1. The SEP-10 server validates the challenge as described in [Token](#token)
1. The SEP-10 server verifies that the challenge transaction is signed by the source account of the `client_domain` Manage Data operation
